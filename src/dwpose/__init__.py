...
import warnings
from .util2 import HWC3, resize_image_with_pad, common_input_validate, annotator_ckpts_path, custom_hf_download
import cv2
...
class DwposeDetector:
    ...
    @classmethod
    def from_pretrained_default(cls,cache_dir=annotator_ckpts_path, torchscript_device="cuda"):
    ...
    def __call__(self, input_image, detect_resolution=512, include_body=True, include_hand=False, include_face=False, include_foot=False, hand_and_face=None, output_type="pil", image_and_json=False, upscale_method="INTER_CUBIC", **kwargs):
        ...
        if image_and_json:
            return (detected_map, encode_poses_as_dict(poses, shape[0], shape[1]),input_image)
